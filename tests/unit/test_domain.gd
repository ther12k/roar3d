...
func _shot_math(h: TestHarness) -> void:
	...
	# Roar Bounce rules (experiment, review round 6): pure math contract.
	h.check_near(BounceRules.restitution_for("turf"), 0.35, 1e-9, "turf restitution")
	h.check_near(BounceRules.restitution_for("spring"), 0.60, 1e-9, "spring restitution")
	h.check_near(BounceRules.restitution_for("dead"), 0.0, 1e-9, "dead surface never bounces")
	h.check_near(BounceRules.restitution_for("mystery"), 0.35, 1e-9, "unknown class is turf")
	h.check(BounceRules.is_landing_normal(Vector3(0, 1, 0)), "flat floor is a landing")
	h.check(BounceRules.is_landing_normal(Vector3(0.3, 1, 0).normalized()), "gentle slope is a landing")
	h.check(not BounceRules.is_landing_normal(Vector3(1, 0, 0)), "wall is not a landing")
	h.check(not BounceRules.is_landing_normal(Vector3(0, -1, 0)), "ceiling is not a landing")
	h.check_near(BounceRules.incoming_normal_speed(Vector3(0, -2, 0), Vector3.UP), 2.0, 1e-9, "falling speed along floor normal")
	h.check_near(BounceRules.incoming_normal_speed(Vector3(0, 2, 0), Vector3.UP), 0.0, 1e-9, "separating velocity is not incoming")
	h.check(not BounceRules.is_eligible(1.19, 0), "below min impact ineligible")
	h.check(BounceRules.is_eligible(1.2, 0), "at min impact eligible")
	h.check(not BounceRules.is_eligible(5.0, 2), "rebound cap exhausted")
	h.check(BounceRules.is_eligible(5.0, 1), "second rebound allowed")
	h.check_near(BounceRules.outgoing_speed(4.0, 0.35, 0.0), 1.4, 1e-9, "turf rebound decays")
	h.check_near(BounceRules.outgoing_speed(4.0, 0.60, 0.0), 2.4, 1e-9, "spring rebound stronger")
	h.check_near(BounceRules.outgoing_speed(4.0, 0.35, 1.6), 3.0, 1e-9, "perfect boost adds along normal")
	h.check_near(BounceRules.outgoing_speed(9.0, 0.60, 1.6), 4.0, 1e-9, "outgoing speed hard-capped")
	h.check(BounceRules.perfect_accepts(0), "press at impact accepted")
	h.check(BounceRules.perfect_accepts(120), "press at window edge accepted")
	h.check(not BounceRules.perfect_accepts(121), "early press expires")
